...
Preserve the separation of `methods/`, `library/`, `examples/`, `problems/`, `instructions/`, `workflows/`, `evaluation/`, `formal/`, and `papers/`.

Treat `papers/` as systems literature for brainstorming agent architectures, Lean tooling, workflow configurations, and evaluation designs. Do not treat it as part of the default mathematical context for benchmark proof attempts.
...
It is allowed to use `papers/` to design coherent workflow permutations, such as combinations of search, refinement, Lean feedback, proof-state tooling, and evaluator loops. Record which papers or software systems inspired a workflow, but keep the resulting workflow specification concise and testable.
