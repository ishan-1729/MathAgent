...
| **Focused prover** | Proves a node → step-ledger (AlphaProof's role). **Default provider is Claude/opus** via the role registry (`orchestrator/registry.py`); Codex / GPT-5.5-xHigh via `codex exec` is the optional provider, selectable per-role in the `RunProfile`. | **built** | AlphaProof_Nexus (AlphaProof tool) |
...
| **Revision controller** | Incumbent tournament; "do nothing" first-class; failure-analysis *before* revision; k=2 stop; margin-gated displacement + a *structural* admissibility gate. **Note:** PUCT + Bradley-Terry here are grafted from `population.py`, **not from AutoReason** (the paper disclaims that game/voting machinery) and are near-decorative — displacement is decided by `net >= margin`. Per-pass aggregation is **pairwise-net, not Borda**. The "admissibility guard" is the *structural* deterministic gate, which does **not** certify elementarity — do not read it as an elementarity enforcer. | **built** ([`orchestrator/tournament.py`](orchestrator/tournament.py); wired via `DagDriver(refiner=…)`; `max_replan_depth` now consumed) | Autoreason (Critic/do-nothing/margin); BT/PUCT from `population.py` |
...
| **Mathlib lemma retrieval** | Loogle (exact names from errors) + a local **BM25 index** + a **neural bi-encoder** (`bge-small-en-v1.5` + optional reranker), combined via `HybridRetriever`. **Default path is lexical (Loogle + BM25).** The neural bi-encoder + cross-encoder reranker are **inert unless the `mathagent[neural]` extra is installed — which the repo does not install/ship**; without it the code uses a non-semantic `HashingEmbedder` (md5 token-hashing, self-labelled "not a real semantic model") and no real neural model ever runs. (Embeds `name+signature` only — no `doc` field is extracted.) | **built; neural leg inert by default** ([`tools/retrieval.py`](tools/retrieval.py), [`tools/semantic_retrieval.py`](tools/semantic_retrieval.py), [`tools/neural_retrieval.py`](tools/neural_retrieval.py)) | LeanSearch/Loogle/LeanExplore, LEAP premise retrieval |
